...
public class VoteProcessor {
    ...
    @RequestProcessing(value = "/vote/down/video", method = HTTPRequestMethod.POST)
    @Before(adviceClass = {LoginCheck.class, PermissionCheck.class})
    public void voteDownVideo(final HTTPRequestContext context, final HttpServletRequest request,
                                final HttpServletResponse response) throws Exception {
        context.renderJSON();
        ...
        if (Vote.TYPE_C_DOWN == vote) {
            ...
            voteMgmtService.voteDown(userId, dataId, Vote.DATA_TYPE_C_VIDEO);
            ...
            if (!VOTES.contains(userId + dataId) && !userId.equals(videoAuthorId)) {
                ...
                notificationMgmtService.addVideoVoteDownNotification(notification);
            }
