...
import org.b3log.symphony.service.TrendsMgmtService;
import org.b3log.symphony.service.TrendsQueryService;
 ...
 */
@WebServlet(urlPatterns = {"/uploadProduct"}, loadOnStartup = 2)
			...
			for (FileItem item : list) {
				if (item.isFormField()) {
					...
					if(suffix.equals("gif")){
						...
						TrendUtils.gifHandle(Symphonys.get("nginx.trend.image.dir")+ret+"/"+filename,Symphonys.get("nginx.trend.image.dir")+ret+"/"+PngImage);
					}else{
						//创建文件
						File file= new File(fileParent,filename);
						//copy
