...
<div class="content admin">
    <div class="module list">
        <form method="GET" action="${servePath}/admin/mall/products" class="form">
            <input name="productNameOrType" type="text" placeholder="${mallProductNameLabel}/${mallProductTypeLabel}"/>
            <button type="submit" class="green">${searchLabel}</button> &nbsp;
            <#if permissions["mallAddProduct"].permissionGrant>
            <button type="button" class="btn red" onclick="window.location = '${servePath}/admin/mall/add-product'">${mallAddProductLabel}</button>
            </#if>
        </form>
        <ul>
        ...
        <@pagination url="${servePath}/admin/mall/products"/>
    </div>
</div>
</@admin>
