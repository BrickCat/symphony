...
var Comment = {
    ...
    remove: function (id) {
        ...
        $.ajax({
            url: Label.servePath + '/comment/' + id + '/remove?type=trend',
            type: "POST",
            cache: false,
    ...
    add: function (id, csrfToken) {
        ...
        if (commentId) {
            url = Label.servePath + "/comment/" + commentId+"?type=trend";
            type = 'PUT';
        }
        $.ajax({
            url: url,
...
$(document).ready(function () {
    ...
    if (Label.isLoggedIn) {
        Trend.makeNotificationRead(Label.trendOId, Label.notificationCmtIds);
