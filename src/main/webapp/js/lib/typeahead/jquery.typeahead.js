...
;(function (factory) {
    ...
    Typeahead.prototype = {
        ...
        buildHtmlLayout: function () {
            ...
            var scope = this,
                groupTemplate = this.groupTemplate || '<ul style="list-style: none;margin-left: -10px;"></ul>',
            ...
            if (this.groupTemplate) {
                groupTemplate = $(groupTemplate.replace(/<([^>]+)>\{\{(.+?)}}<\/[^>]+>/g, function (match, tag, group, offset, string) {
                    ...
                    if (!scope.result.length) {
                        ...
                        return '<' + tag + ' class="' + scope.options.selector.empty + '"><p>' + emptyTemplate + '</p></' + tag + '>';
                    }
                ...
                if (!this.result.length) {
                    groupTemplate.append(
                        emptyTemplate instanceof $ ?
                            emptyTemplate :
                            '<li class="' + scope.options.selector.empty + '" style=""><p>' + emptyTemplate + '</p></li>'
                    );
                }
            }
